...
Merged_Word_Counts = defaultdict(init_nested_default_dict)
...
for node_counts in Sync_Class_Word_Counts:
    for label, word_counts in node_counts.items():
        for word, count in word_counts.items():
            Merged_Word_Counts[label][word] = Merged_Word_Counts[label][word] + count
...
# all words
for word_counts in Merged_Word_Counts.values():
...
Vocab = sorted([word for word in Vocab if Word_Counter[word] > 1])
...
for label in sorted(Merged_Class_Counts.keys()):
    word_probs = [ (Merged_Word_Counts[label][word]) / (Merged_Class_Counts[label]) for word in Vocab]
